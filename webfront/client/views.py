...
import urllib
from webfront.client.forms import PenaltyForm, NoticeForm
...
@cache_page(15*60)
@render('webfront/client/client.html')
...
def _get_banlist(request):
    cache_key = "%s_banlist" % request.session.get('server')
...
@login_required
@cache_page(120*60)
@render('webfront/client/world_map.html')
...
@cache_page(15*60)
@render('webfront/client/admin_list.html')
def adminlist(request, filter=False):
    clients = Client.objects.using(request.session.get('server')).filter(id__gt=1, group__level__gte=settings.HIGH_LEVEL_CLIENT)
    ...
    if request.POST.has_key('days') or request.GET.has_key('days') or filter:
        ...
        res['render_template'] = 'webfront/client/inactiveadmin_list.html'
        res['days']=int(days)
...
@permission_required_with_403('b3connect.view_client')
@cache_page(15*60)
@render('webfront/client/client_list.html')
@flood
def clientlist(request):
    ...
    if request.GET.has_key('searchall') and request.user.has_perm('b3connect.client_advanced_search'):
        ...
        for server in request.server_list:
            list[server['NAME']] = _getclientlist(request, server['DB'])
        ...
        clients = _getclientlist(request, request.session.get('server'))
    else:
        clients = _getclientlist(request, request.session.get('server'), False)
...
@permission_required_with_403('b3connect.view_client')
@cache_page(15*60)
@render('webfront/client/regular_client_list.html')
@flood
def regularclients(request):
    dt = datetime.datetime.now() - datetime.timedelta(days=7)
    clients = Client.objects.using(request.session.get('server')).filter(id__gt=1, connections__gte=50, time_edit__gte=time.mktime(dt.timetuple()))
    clients = clients.order_by('-time_edit')
...
def disablepenalty(request, id):
    penalty = get_object_or_404(Penalty, id=id, using=request.session.get('server'))
    penalty.inactive = 1
    penalty.save()
...
def editpenalty(request, id):
    p = get_object_or_404(Penalty, id=id, using=request.session.get('server'))
    if request.method == 'POST':
...
def change_clientgroup(request, id):
    ...
    group = get_object_or_404(Group, id=int(g), using=request.session.get('server'))
    client = get_object_or_404(Client, id=id, using=request.session.get('server'))
...
@permission_required_with_403('b3connect.view_client')
@cache_page(15*60)
@render('webfront/client/client_aliases.html')
...
@cache_page(15*60)
@render('webfront/client/client_adminactions.html')
...
@permission_required_with_403('b3connect.view_client')
@cache_page(15*60)
@render('webfront/client/client_penalties.html')
...
@permission_required_with_403('b3connect.view_client')
@cache_page(15*60)
@render('webfront/client/client_notices.html')
